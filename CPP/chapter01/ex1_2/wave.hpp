...
public:
    ...
    int bits; /* 量子化精度 */
    int length; /* 音データの長さ */
    std::vector<double> sL; /* 音のデータ(Lチャンネル) */
    std::vector<double> sR; /* 音のデータ(Rチャンネル) */
};

void wave_read_8bit_mono(MONO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    ...
    pcm->s.reserve(pcm->length); /* メモリの確保 */
...
void wave_write_8bit_mono(MONO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
...
void wave_read_8bit_stereo(STEREO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
    ...
    pcm->sL.reserve(pcm->length); /* メモリの確保 */
    pcm->sR.reserve(pcm->length); /* メモリの確保 */
...
void wave_write_8bit_stereo(STEREO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
...
void wave_read_16bit_mono(MONO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
    ...
    for (auto &val : pcm->s)
    {
        ...
        val = (double)data / 32768.0; /* 音データを-1以上1未満の範囲に正規化する */
    }

    fclose(fp);
}

void wave_write_16bit_mono(MONO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
...
void wave_read_16bit_stereo(STEREO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
...
void wave_write_16bit_stereo(STEREO_PCM *pcm, std::string file_name)
{
    FILE *fp;
    char riff_chunk_ID[4];
    long riff_chunk_size;
